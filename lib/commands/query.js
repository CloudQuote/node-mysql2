...
class Query extends Command {
  constructor(options, callback) {
    ...
    this.useFastTextParser = options.useFastTextParser || false;
  }
  ...
  readField(packet, connection) {
    ...
    if (this._receivedFieldsCount === this._fieldCount) {
      ...
      this._rowParser = this.useFastTextParser
        ? new (getFastTextParser(fields, this.options, connection.config))(
            fields
          )
        : new (getTextParser(fields, this.options, connection.config))(fields);
  ...
  stream(options) {
    ...
    stream.once('error', err => {
      ...
      // if the readable stream is sent an error, propagate the error to the connection
      if (this._connection.release) this._connection.release(); // For pooled connections
      if (this._connection.destroy) this._connection.destroy(); // For pooled & non-pooled connections
